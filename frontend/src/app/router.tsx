...
export const router = createBrowserRouter([
  { path: "/login", element: <LoginPage /> },
  {
    ...
    children: [
      ...
      { path: "admin/departments", element: <ResourcePage title="Departments" path="/departments" fields={["name", "abbreviation", "course_id", "description", "low_attendance_threshold"]} fieldTypes={{ description: "text", low_attendance_threshold: "number" }} relations={[{ field: "course_id", labelText: "Course", path: "/courses", label: (item) => `${String(item.abbreviation ?? "")} - ${String(item.name ?? "")}`.trim() }]} importable exportable templateable /> },
      ...
      { path: "vision", element: <Navigate to="/monitoring" replace /> },
      { path: "reports", element: <ReportsPage /> },
      { path: "notifications", element: <NotificationsPage /> },
      { path: "search", element: <SearchPage /> },
      { path: "courses", element: <ResourcePage title="Courses" path="/courses" fields={["name", "abbreviation", "duration_years", "is_active"]} fieldTypes={{ duration_years: "number", is_active: "boolean" }} importable exportable templateable /> },
      { path: "subjects", element: <ResourcePage title="Subjects" path="/subjects" fields={["code", "name", "course_id", "department_id", "semester", "credits", "is_active"]} fieldTypes={{ semester: "number", credits: "number", is_active: "boolean" }} relations={[{ field: "course_id", labelText: "Course", path: "/courses", label: (item) => `${String(item.abbreviation ?? "")} - ${String(item.name ?? "")}`.trim() }, { field: "department_id", labelText: "Department", path: "/departments", label: (item) => `${String(item.abbreviation ?? "")} - ${String(item.name ?? "")}`.trim() }]} importable exportable templateable /> },
      { path: "subject-assignments", element: <ResourcePage title="Subject Assignments" path="/subject-assignments" fields={["faculty_id", "subject_id", "section", "academic_year", "is_active"]} fieldTypes={{ is_active: "boolean" }} relations={[{ field: "faculty_id", labelText: "Faculty", path: "/faculty", label: (item) => `${String(item.employee_id ?? "")} - ${String(item.user?.full_name ?? "")}`.trim() }, { field: "subject_id", labelText: "Subject", path: "/subjects", label: (item) => `${String(item.code ?? "")} - ${String(item.name ?? "")}`.trim() }]} importable exportable templateable /> },
      { path: "classrooms", element: <ResourcePage title="Classrooms" path="/classrooms" fields={["name", "building", "capacity", "camera_url"]} fieldTypes={{ capacity: "number" }} importable exportable templateable /> }
    ]
  }
]);
